...
uses
  ...
  {$ENDIF}
  EncdDecd, Data.DBXPlatform, FMX.Graphics, System.Classes, System.SysUtils;
...
type
  ...
  public
    ...
    (* 208 Reserved Content *)
    class function RESERVED_CONTENT: TReponseCode;
  ...
  public
    ...
    class procedure responseServer(const Response: TReponseCode; Content : string = '');
  end;
...
class function TStatusCode.RESERVED_CONTENT: TReponseCode;
begin
  Result.StatusCode := 208;
  Result.Reason := 'reserved Content';
end;
...
class function TRestUtils.BitmapFromBase64(const base64: string): TBitmap;
var
  Input: TStringStream;
  Output: TBytesStream;
begin
...
class procedure TRestUtils.responseServer(const Response: TReponseCode; Content : string = '');
begin
  GetInvocationMetadata.ResponseCode    := Response.StatusCode;
  GetInvocationMetadata.ResponseMessage := Response.Reason;

  if Content = '' then
    GetInvocationMetadata.ResponseContent := Response.Reason
  else
    GetInvocationMetadata.ResponseContent := '{"MENSAGEM":"ERRO CRÍTICO."}';
end;
